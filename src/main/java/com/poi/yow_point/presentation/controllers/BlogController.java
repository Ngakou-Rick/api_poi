...
public class BlogController {
    ...
    public Mono<BlogDTO> updateBlog(
            @Parameter(description = "ID of the blog post to update", required = true) @PathVariable UUID blogId,
        ...
        log.info("REST request to update blog post: {}", blogId);
    ...
    public Mono<BlogDTO> getBlogById(
            @Parameter(description = "Unique ID of the blog post", required = true) @PathVariable UUID blogId) {
        log.debug("REST request to get blog post by ID: {}", blogId);
    ...
    public Flux<BlogDTO> getBlogsByUserId(
            @Parameter(description = "ID of the user who wrote the blog posts", required = true) @PathVariable UUID userId) {
        log.debug("REST request to get blog posts for user: {}", userId);
    ...
    public Flux<BlogDTO> getBlogsByPoiId(
            @Parameter(description = "ID of the Point of Interest the blog posts are associated with", required = true) @PathVariable UUID poiId) {
        log.debug("REST request to get blog posts for POI: {}", poiId);
        return blogService.getBlogsByPoiId(poiId);
    }
    ...
    public Mono<Void> deleteBlog(
            @Parameter(description = "ID of the blog post to delete", required = true) @PathVariable UUID blogId) {
        log.info("REST request to delete blog post: {}", blogId);
        return blogService.deleteBlog(blogId);
    }

    @GetMapping("/count/user/{userId}")
    @Operation(summary = "Count blog posts by user", description = "Gets the total number of blog posts written by a specific user.")
    @ApiResponse(responseCode = "200", description = "Total number of blog posts for the user", content = @Content(schema = @Schema(implementation = Long.class, example = "7")))
    ...
    @Operation(summary = "Count blog posts by POI", description = "Gets the total number of blog posts associated with a specific POI.")
    @ApiResponse(responseCode = "200", description = "Total number of blog posts for the POI", content = @Content(schema = @Schema(implementation = Long.class, example = "3")))
